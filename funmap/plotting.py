...
def plot_llr_comparison(
    cfg, validation_results, llr_ds, output_file="llr_comparison.pdf"
):
    name_type_dict = {item["name"]: item["type"] for item in cfg["data_files"]}
    name_type_dict["all_average"] = "other"
    ...
    line_styles = [
        Line2D([0], [0], linestyle="--", color="black", label="RNA"),
        Line2D([0], [0], linestyle=":", color="black", label="Protein"),
        Line2D([0], [0], linestyle="-.", color="black", label="Extra Feature"),
        Line2D([0], [0], linestyle="-", color="black", label="Other"),
    ]
    color_legend = ax.legend(
        handles=[
            Patch(color=line.get_color(), label=line.get_label()) for line in ax.lines
        ],
    ...
    ax.set_xlabel("number of pairs", fontsize=16)
    ...
    plt.savefig(
        output_file,
        bbox_inches="tight",
        bbox_extra_artists=[color_legend, linestyle_legend],
    )
    plt.close(fig)
    return output_file
...
def explore_data(data_config: Path, min_sample_count: int, output_dir: Path):
    ...
    for ds in data_dict:
        ...
        if data_df.shape[0] > max_col_to_plot:
            ax[0].set_xlabel(
                f"random selected {max_col_to_plot} samples (total n={data_df.shape[0]})"
            )
...
def plot_results(cfg, validation_results, llr_ds, gs_dict, cutoff_llr, figure_dir):
    fig_names = []
    file_name = "llr_compare_dataset.pdf"
    plot_llr_comparison(
        cfg, validation_results, llr_ds, output_file=figure_dir / file_name
    )
    fig_names.append(file_name)

    if "rp_pairs" in cfg:
        file_names = plot_pair_llr(
            gs_dict,
            cfg["feature_type"],
            output_dir=figure_dir,
            rp_pairs=cfg["rp_pairs"],
        )
        fig_names.extend(file_names)

    if cfg["task"] == "protein_func":
        file_name = "llr_compare_networks.pdf"
        # if gs_file is not specified, plot the llr comparison between networks
        # because some the numbers are pre-computed based on default gs_file
        if cfg["gs_file"] is None:
            plot_llr_compare_networks(
                validation_results,
                cfg["lr_cutoff"],
                cutoff_llr,
                output_file=figure_dir / file_name,
            )
            fig_names.append(file_name)
        ...
        network_info["el"] = network_info["url"].apply(
            lambda x: pd.read_csv(x, sep="\t", header=None)
        )
        ...
        for ft in validation_results:
            ...
            funmap_df = pd.DataFrame(
                {"name": ["FunMap"], "type": ["FunMap"], "el": [funmap_el]}
            )
            ...
            for type, color in zip(["node", "edge"], [node_color, edge_color]):
                fig_name = plot_overlap_venn(
                    f"funmap_{ft}", overlap_info[type], type, color, figure_dir
                )
...
def plot_1d_llr(ax, feature_df, feature_name, feature_type, data_type, n_bins):
    ...
    if data_type == "PRO":
        ...
        ax.set_ylim(data_range[feature_type])
        ax.set_xlim(0, 2.5)
    else:
        ax.hist(
            cur_df_vals,
        ...
        ax.set_xlim(data_range[feature_type])
        ax.set_ylim(0, 2.5)
...
def plot_2d_llr(
    ...
    feature_label_mapping = {"CC": "correlation coefficient", "MR": "mutual rank"}
    cnt = {}
    cnt_pos_neg = {}
    max_density = -1
    ...
    for label in [0, 1]:
        ...
        hist_density, _, _ = np.histogram2d(
            cur_df[rna_feature].values,
            cur_df[pro_feature].values,
            bins=n_bins,
            range=np.array([data_range[feature_type], data_range[feature_type]]),
            density=True,
        )
    ...
    llr_vals = ((cnt_pos_neg[1] + 1) / (cnt_pos_neg[0] + cnt[0] / cnt[1])) / (
        cnt[1] / cnt[0]
    )
    if feature_type == "MR":
        vmin = np.percentile(llr_vals, 5)
        vmax = np.percentile(llr_vals, 95)
        symmetric_max = max(abs(vmin), abs(vmax))
        vmin = -symmetric_max
        vmax = symmetric_max
    else:
        vmin = -4
    ...
    cmap = plt.cm.RdBu_r
    fig = ax.pcolormesh(
        hh[1], hh[2], np.transpose(np.log(llr_vals)), vmin=vmin, vmax=vmax, cmap=cmap
    )
    ax.text(
        0.02,
        0.01,
    ...
    ax.set_xlabel(
        f"{label_mapping[data_types[0]]}\n{feature_label_mapping[feature_type]}",
        fontsize=16,
    )
    ax.set_ylabel(
        f"{label_mapping[data_types[1]]}\n{feature_label_mapping[feature_type]}",
        fontsize=16,
    )
    if feature_type == "CC":
...
def plot_pair_llr(gs_dict, feature_type, output_dir, rp_pairs):
    n_bins = 20
    file_names = []
    feature_type = ["CC", "MR"]
...
def plot_llr_compare_networks(validaton_results, cutoff, cutoff_llr, output_file):
    all_networks = []

    for ft in validaton_results:
        edge_list = pd.read_csv(
            validaton_results[ft]["edge_list_path"], sep="\t", header=None
        )
        n_edge = len(edge_list)
        n_node = len(set(edge_list.iloc[:, 0]) | set(edge_list.iloc[:, 1]))
        all_networks.extend(
            [
                (
                    f"FunMap_{ft}",
    ...
    mycmap = matplotlib.colors.ListedColormap(
        ["#de2d26", "#8B6CAF", "#0D95D0", "#69A953", "#F1C36B", "#DC6C43"]
    )
    # group 0 is FunMap, group 1 is HI, group 2 is ProHD, group 3 is STRING,
    # group 4 is BioGRID, group 5 is BioPlex
    # the length of gro
    color_group = (
        [0] * len(validaton_results) + [1] * 2 + [2] * 1 + [3] * 1 + [4] * 1 + [5] * 1
    )
    scatter = ax.scatter(x, y, c=color_group, cmap=mycmap, s=e / 1000)
    ax.set_ylim(2, 6)
    ...
    handles1, labels1 = scatter.legend_elements(
        prop="sizes", num=4, alpha=1, fmt="{x:.0f} K"
    )
    legend1 = ax.legend(
        handles1,
        labels1,
...
def plot_overlap_venn(network_name, overlap, node_or_edge, color, output_dir):
    ...
    for i, nw in enumerate(overlap):
        ...
        venn2_circles(
            overlap[nw], ax=cur_ax, linestyle="solid", color="gray", linewidth=1
        )
        if out.set_labels:
...
def plot_network_stats(network_info, feature_type, output_dir):
    ...
    for n in network_list:
        ...
        if n == "FunMap":  # only fit for FunMap
            with warnings.catch_warnings():
                ...
                fit = powerlaw.Fit(
                    cur_degrees, discrete=True, xmax=250, estimate_discrete=False
                )
                powerlaw.plot_pdf(
                    cur_degrees,
                    linear_bins=True,
    ...
    # these are pre-calculated since they take a long time to compute and
    # the network is fixed
    avg_cc = {"BioGRID": 0.125, "BioPlex": 0.103, "HI-union": 0.06, "STRING": 0.335}
    for n in network_list:
        network_el = network_info.loc[network_info["name"] == n, "el"].values[0]
        cur_network = nx.from_pandas_edgelist(network_el, source=0, target=1)
    ...
    ax[1].bar(
        network_list,
        [avg_cc[i] for i in network_list],
        width=0.5,
        align="center",
        color="#E4C89A",
    )
    ...
    ax[2].bar(
        network_list,
        [density[i] for i in network_list],
        width=0.5,
        align="center",
        color="#D8B2C6",
    )
    ...
    ax[3].bar(
        network_list,
        [average_shortest_path[i] for i in network_list],
        width=0.5,
        align="center",
        color="#B6D8A6",
    )
