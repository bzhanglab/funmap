...
def get_valid_gs_data(gs_path: str, valid_gene_list: List[str], md5=None):
    ...
    if is_url_scheme(gs_path):
        gs_edge_df = read_csv_with_md5_check(
            gs_path, expected_md5=md5, local_path="download_gs_file", sep="\t"
        )
    ...
    gs_edge_df = gs_edge_df.rename(
        columns={gs_edge_df.columns[0]: "P1", gs_edge_df.columns[1]: "P2"}
    )
    gs_edge_df = gs_edge_df[
        gs_edge_df["P1"].isin(valid_gene_list)
...
def pairwise_mutual_rank(pcc_matrix):
    ...
    rank_ab = (
        np.argsort(pcc_matrix, axis=1).argsort(axis=1, kind="mergesort") + 1
    )  # Start ranks from 1
    rank_ba = (
        np.argsort(pcc_matrix, axis=0).argsort(axis=0, kind="mergesort") + 1
    )  # Start ranks from 1
    ...
    mr_values = np.sqrt(
        (rank_ab[valid_indices_a, valid_indices_b] / n_b[valid_indices_b])
...
def assemble_feature_df(h5_file_mapping, df, dataset="cc"):
    ...
    for key, file_path in h5_file_mapping.items():
        with h5py.File(file_path, "r") as h5_file:
            ...
            p1_indices = np.array(
                [gene_to_index.get(gene, -1) for gene in df.iloc[:, 0]]
            )
            p2_indices = np.array(
                [gene_to_index.get(gene, -1) for gene in df.iloc[:, 1]]
            )
...
def get_ppi_feature():
    ...
    urls = [
        network_info["url"][i]
        for i in range(len(network_info["name"]))
...
def train_model(X, y, seed, n_jobs, feature_mapping, model_dir):
    ...
    for ft in feature_mapping:
        ...
        clf = GridSearchCV(
            xgb_model, model_params, scoring="roc_auc", cv=cv, n_jobs=1, verbose=2
        )
        if ft == "ex":
...
def compute_llr(
    ...
    log.info(
        f"Total predictions: {total_predictions:,}, Valid (non-NaN) predictions: "
    ...
    gs_test_pos_set = set(
        gs_test[gs_test["label"] == 1][["P1", "P2"]].apply(
            lambda row: tuple(sorted({row["P1"], row["P2"]})), axis=1
        )
    )
    gs_test_neg_set = set(
        gs_test[gs_test["label"] == 0][["P1", "P2"]].apply(
            lambda row: tuple(sorted({row["P1"], row["P2"]})), axis=1
        )
    )
    ...
    for k in tqdm(
        range(start_edge_num, max_num_edges + step_size, step_size),
        total=total,
        ascii=" >=",
    ):
        ...
        try:
            lr = (
                len(common_pos_edges)
                / len(common_neg_edges)
                / (n_gs_test_pos_set / n_gs_test_neg_set)
            )
...
def dataset_llr(
    ...
    for dataset in feature_dict:
        ...
        predicted_all_pairs = extract_dataset_feature(
            all_pairs, feature_file, feature_type
        )
        if all_ds_pred is None:
            all_ds_pred = predicted_all_pairs["prediction"].values
        else:
            all_ds_pred = np.vstack(
                (all_ds_pred, predicted_all_pairs["prediction"].values)
            )

        cur_llr_res = compute_llr(
            predicted_all_pairs, None, start_edge_num, max_num_edge, step_size, gs_test
        )
...
def get_cutoff(model_dict, gs_test, lr_cutoff):
    ...
    for ft in model_dict:
        ...
        if cutoff_prob is None:
            log.error(
                f"Cannot find cutoff prob for {ft}, lower lr_cutoff and try again"
            )
            import sys
    ...
    for ft in model_dict:
        ...
        with ThreadPoolExecutor(max_workers=n_jobs) as executor:
            for chunk_id, chunk_start in enumerate(
                range(0, len(all_pairs), chunk_size)
            ):
        ...
        matching_files.sort()
        pq.write_table(
            pa.concat_tables([pq.read_table(file) for file in matching_files]),
            output_file[ft],
        )
        for file in matching_files:
            os.remove(file)
