...
def qc(config_file, force_rerun):
    if force_rerun:
        while True:
            confirmation = input(
                "Do you want to remove results from previous run? (y/n): "
            )
...
def run(config_file, force_rerun):
    ...
    if (extra_feature_file is not None) and (
        not check_extra_feature_file(extra_feature_file)
    ):
        return
    gs_file = cfg["gs_file"]
    ...
    ml_model_file = {
        feature: model_dir / f"model_{feature}.pkl.gz" for feature in feature_mapping
    }
    predicted_all_pairs_file = {
        feature: prediction_dir / f"predicted_all_pairs_{feature}.parquet"
        for feature in feature_mapping
    }
    llr_res_file = {
        feature: results_dir / f"llr_results_{feature}.tsv"
        for feature in feature_mapping
    }
    edge_list_file = {
        feature: network_dir / f"funmap_{feature}.tsv" for feature in feature_mapping
    }
    ...
    all_edge_list_exist = all(
        os.path.exists(file_path) for file_path in edge_list_file.values()
    )

    if all_edge_list_exist:
        log.info(
            "Fumap network(s) already exists. Skipping model training and prediction."
        )
    else:
        all_model_exist = all(
        ...
        if all_model_exist:
            ...
            ml_model_dict = train_ml_model(
                gs_train, ml_type, seed, n_jobs, feature_mapping, model_dir
            )

        all_predicted_all_pairs_exist = all(
            os.path.exists(file_path) for file_path in predicted_all_pairs_file.values()
        )
    ...
    all_llr_res_exist = all(
        os.path.exists(file_path) for file_path in llr_res_file.values()
    )
    all_edge_list_exist = all(
        os.path.exists(file_path) for file_path in edge_list_file.values()
    )
    if all_llr_res_exist and all_edge_list_exist:
        ...
        for ft in feature_mapping:
            ...
            if not predicted_all_pairs_file[ft].exists():
                log.error(
                    f"Predicted all pairs file {predicted_all_pairs_file[ft]} does not exist."
                )
                return
            ...
            if max_num_edges >= num_valid:
                log.warning(
                    f"max_num_edges ({max_num_edges:,}) is >= number of valid predictions "
    ...
    fig_names = plot_results(
        cfg, validation_res, llr_ds, gs_dict, cutoff_llr, figure_dir
    )
    all_fig_names.extend(fig_names)
