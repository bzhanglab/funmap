...
def read_csv_with_md5_check(
    url, expected_md5=None, local_path="downloaded_file.csv", **kwargs
):
    try:
        ...
        if expected_md5:
            ...
            if md5_hash != expected_md5:
                log.error(
                    "gold standard file: MD5 hash mismatch, file may be corrupted."
                )
    ...
    if config["extra_feature_file"] is not None:
        ...
        if config["filter_noncoding_genes"]:
            ...
            extra_feature_df = extra_feature_df[
                extra_feature_df[col1].isin(coding_genes)
                & extra_feature_df[col2].isin(coding_genes)
            ]
...
def get_node_edge_overlap(network_info):
    ...
    target = "FunMap"
    cur_res = {}
    target_node_set = set(networks.loc[networks["name"] == target, "nodes"].tolist()[0])
    ...
    for _, row in networks.iterrows():
        ...
        cur_res[row["name"]] = tuple(
            [target_size - overlap_size, cur_size - overlap_size, overlap_size]
        )
    ...
    target_edge_set = set(
        tuple(sorted(x)) for x in zip(target_edge_df.pop(0), target_edge_df.pop(1))
    )
...
def setup_experiment(config_file):
    ...
    folders = [
        results_dir / Path(folder_dict[folder_name]) for folder_name in folder_dict
    ]
...
def check_gold_standard_file(file_path, min_count=10000):
    ...
    with open(file_path, "r", newline="") as tsv_file:
        ...
        for row_num, row in enumerate(
            reader, start=2
        ):  # Add row_num for better error reporting
            if len(row) != 3:  # Assuming each row should have 3 columns
                log.error(
                    f"Invalid row format in row {row_num}. Each row should have 3 columns."
                )
                return False
    ...
    if lesser_count < min_count:
        log.error(
            f"The lesser of 0 and 1 occurrences ({lesser_count}) does not meet the threshold. "
    ...
    with open(file_path, "r", newline="") as tsv_file:
        ...
        for row_num, row in enumerate(
            ...
            for column_name, value in row.items():
                if (
                    column_name not in header[:2]
                ):  # Skip the first two columns (Protein_1 and Protein_2)
                    ...
                    try:
                        ...
                        log.error(
                            "Invalid data type in row %d, column '%s'. "
                    ...
                    if column_name not in column_data_types:
                        ...
                        log.error(
                            "Inconsistent data type in column '%s'. "
